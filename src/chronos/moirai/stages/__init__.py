...
only stage that finalizes N). Phase 4c adds the first three re-run gates: 4.5 cost
stress, 4.6 capacity, 4.7 shifted-window stability — all sharing the one
`moirai/rerun.py` VERIFICATION re-run helper. Each reads the BacktestResult and/or
the store, computes, compares to a config threshold, and returns a TestOutcome. The
remaining re-run stages (4.8–4.10) arrive in the next session.
...
__all__ = [
    ...
    "CostStress", "Capacity", "ShiftedWindow",
]
